...
// Prepare an array to store the class IDs
$class_ids = [];
while ($row = $classResult->fetch_assoc()) {
    $class_ids[] = $row['class_id'];
}

// Fetch the students who belong to the same class as the teacher
$query = "SELECT s.user_id, s.username
          FROM users s
          JOIN student_class sc ON sc.student_id = s.user_id
          WHERE sc.class_id IN (" . implode(',', $class_ids) . ")";
$stmt = $conn->prepare($query);
$stmt->execute();
$studentsResult = $stmt->get_result();

// Fetch all subjects (this is used to display the subject names)
$subjects = [];
...
<div class="footer">
    <p>&copy; 2025 Moduli Mësuesit. Të gjitha të drejtat janë të rezervuara.</p>
</div>
